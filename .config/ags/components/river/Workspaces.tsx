...
const Displays = () => {
    ...
    return <box vertical valign={START}>
        {outputs.get().map(v =>
...
const TagButtons = ({ outputName }: { outputName: string }) => {
    ...
    return <box>
        ...
        <label label={bind(output, "focused_tags").as(v => String(v)).get()} />
        {new Array(3).fill(null).map((_, i) => {
            return <button label={`${i}`} onClick={() => output.focused_tags = 1 << i} />
        })}
    </box>
}
...
export const Workspaces = () => {
    ...
    return <box className="River">
        {outputs.as(v => v.map(v => {
