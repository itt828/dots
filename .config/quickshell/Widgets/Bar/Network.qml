...
Item {
    ...
    IconLabel {
        ...
        text: {
            ...
            if (!networkService.isWifiEnabled)
                return "Disabled";
            return "Disconnected";
        }
        ...
        icon: {
            if (networkService.isConnected) {
                if (networkService.isWired)
                    return FontIcons.network;

                if (networkService.isWifi) {
                    var signal = networkService.signalStrength;
                    if (signal > 80)
                        return FontIcons.wifiHigh;
                    if (signal > 50)
                        return FontIcons.wifiMedium;
                    if (signal > 20)
                        return FontIcons.wifiLow;
